...
def get_outputs_dir_from_path(outdir_base, path):
    ...
    return os.path.join(outdir_base, f'{view_id}')
    ...
    def process_view_dir(dir_pair):
        ...
        for filename in os.listdir(rendered_dir):
            ...
            try:
                ...
                rgb_path = os.path.join(rgb_dir, f'{cam_id}.exr')
                ...
                depth_path = os.path.join(depth_dir, f'{cam_id}.exr')
                save_as_exr(rgb_path, rgb)
                depth_expanded = np.repeat(depth[:, :, np.newaxis], 3, axis=2)
                save_as_exr(depth_path, depth_expanded)
                ...
                cam_data = np.load(os.path.join(camera_dir, f'Camera_0.npz'))
                K = cam_data['K']
                Rt = cam_data['Rt']
